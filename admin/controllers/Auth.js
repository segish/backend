    ...
    jwt.verify(token, process.env.JWT_SECRETE_KEY, async (err, userInfo) => {
        ...
        try {
            ...
            if (req.body.type === "admin") {
                ...
                if (existingShop) return res.status(403).json("A cshier account already exists for this shop");
...
const login = async (req, res) => {
    try {
        ...
        if (!admin) return res.status(404).json("Incorrect email or password");
...
const getAdmin = async (req, res) => {
    ...
    jwt.verify(token, process.env.JWT_SECRETE_KEY, async (err, userInfo) => {
        if (err) return res.clearCookie("adminAccessToken", {
            secure: true,
            httpOnly: true,
            sameSite: "strict",
        }) .status(403).json("Some thing went wrong please Logout and Login again ");
